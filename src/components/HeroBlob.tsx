...
function Blob() {
  ...
  useFrame((state) => {
    ...
    material.uniforms.u_intensity.value = MathUtils.lerp(
      ...
      hover.current ? 0.65 : 0.45,
      0.02,
    );

    targetPosition.current.set(mouse.x * 0.6, mouse.y * 0.6, 0);
...
export default function HeroBlob({ fallback }: HeroBlobProps) {
  return (
    <Canvas className="h-full w-full" camera={{ position: [0, 0, 8] }} dpr={[1, 1.25]} gl={{ antialias: false }}>
      <ambientLight intensity={0.85} />
      <directionalLight position={[4, 6, 8]} intensity={0.9} />
      <directionalLight position={[-6, -4, -2]} intensity={0.35} />
      <Suspense
        fallback={
          fallback ?? <div className="h-full w-full animate-pulse rounded-full bg-[#4F46E5]/10" />
        }
      >
        <Blob />
      </Suspense>
